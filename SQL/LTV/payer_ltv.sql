/* Calculate the cohorted LTV for payers up to 120-days after the install date. 
...
SELECT acquired_at::date,
       count(distinct advertising_id) AS "Purchasers",
       sum(CASE
       ...
       sum(CASE
               WHEN date_diff('sec',acquired_at,created_at)/86400 <= 120 then NULL
               ELSE revenue/100.0
